...
export const GameView: React.FC<GameViewProps> = ({ currentUser, onOpenDeposit }) => {
  ...
  const activeBetId1Ref = useRef<string | null>(null);
  const activeBetId2Ref = useRef<string | null>(null);
  ...
  const applyAuthoritativeBets = useCallback(async (roundId: string) => {
    try {
      ...
      if (p1) activeBetId1Ref.current = p1.id;
      if (p2) activeBetId2Ref.current = p2.id;
  ...
  const reloadWallet = useCallback(async () => {
    try {
      ...
      setWallet({
        ...
        totalBalance: Number(data.available_balance) + Number(data.locked_balance),
        currency: data.currency || 'USD'
      });
    } catch (e) {
      console.warn('[GameView] reloadWallet error:', e);
    }
  }, [currentUser.id]);
  ...
  const handleCashOut = useCallback(async (panelId = 1) => {
    ...
    const betIdRef = isP1 ? activeBetId1Ref : activeBetId2Ref;
    ...
    try {
      let targetBetId = betIdRef.current;
      if (!targetBetId) {
        const own = store.getActiveBets().find(b => b.isCurrentUser && b.status === 'active' && b.panelId === panelId);
        targetBetId = own?.id || null;
      }
      if (!targetBetId) throw new Error('BET_NOT_FOUND_IN_CLIENT_STATE');
      const result = await authoritativeCashout(targetBetId);
  ...
  const handlePlaceBet = useCallback(async (amount: number, autoCashOut: number | null, panelId = 1) => {
    ...
    try {
      ...
      if (isP1) {
        activeBetId1Ref.current = result.bet_id;
        hasCashedOut1Ref.current = false; lastBetAmount1Ref.current = amount; setHasActiveBet1(true); setBetAmount1(amount); setIsQueued1(false); setQueuedBet1(null); setHasCashedOut1(false); setCashedOutMultiplier1(null); setCashedOutPayout1(null);
      } else {
        activeBetId2Ref.current = result.bet_id;
        hasCashedOut2Ref.current = false; lastBetAmount2Ref.current = amount; setHasActiveBet2(true); setBetAmount2(amount); setIsQueued2(false); setQueuedBet2(null); setHasCashedOut2(false); setCashedOutMultiplier2(null); setCashedOutPayout2(null);
      }
      await reloadWallet();
  ...
  useEffect(() => {
    const unsubscribe = subscribeToAuthoritativeRound(async (serverRound) => {
      const mapped: GameRound = {
        ...
        scheduledStartAt: serverRound.scheduledStartAt ? new Date(serverRound.scheduledStartAt).getTime() : null,
      ...
      if (serverRound.status === 'COUNTDOWN' || serverRound.status === 'WAITING') {
        if (serverRound.roundNumber !== lastAutoBetRoundRef.current) {
          lastAutoBetRoundRef.current = serverRound.roundNumber;
          triggerAutoBets();
        }
      }
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      const authRound = {
        ...
        scheduledStartAt: round.scheduledStartAt ? new Date(round.scheduledStartAt).toISOString() : null,
