 ...
 * the source of truth; this is a bounded presentation cache.
 */
...
function mapRow(row: any): GameRound {
  return {
    id: String(row.id), roundNumber: Number(row.round_number), status: row.status,
    ...
    crashPoint: Number(row.crash_point), serverSeed: undefined,
    serverSeedHash: row.server_seed_hash || '', clientSeed: row.client_seed || '',
    nonce: Number(row.nonce || 0), totalBetsAmount: Number(row.total_bets_amount || 0),
...
if (isSupabaseConfigured) {
  ...
  const unsubscribe = subscribeToAuthoritativeRound((round) => {
    if (round.status === 'CRASHED' || round.status === 'SETTLED') void refreshHistory();
  }, 500);
