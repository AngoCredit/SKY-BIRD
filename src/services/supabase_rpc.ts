...
export async function fetchUserBetsHistory(userId: string): Promise<any[]> {
  if (!isSupabaseConfigured || !userId || userId === 'usr_guest') return [];
  try {
    const uuid = toValidUuid(userId);
    const query = uuid
      ? supabase.from('bets').select('*').eq('user_id', uuid).order('created_at', { ascending: false }).limit(50)
      : supabase.from('bets').select('*').eq('user_id', userId).order('created_at', { ascending: false }).limit(50);
    const { data, error } = await query;
    if (error) {
      console.warn('[Supabase] Erro ao buscar histórico de apostas:', error.message);
      return [];
    }
    return data || [];
  } catch (e) {
    console.warn('[Supabase] Exceção ao buscar histórico de apostas:', e);
    return [];
  }
}
