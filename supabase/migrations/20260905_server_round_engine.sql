...
BEGIN
  ...
  IF r.status IN ('CRASHED','SETTLED') THEN
    IF r.started_at IS NULL OR clock_timestamp() - r.ended_at >= interval '5 seconds' THEN
      PERFORM public.create_next_round();
  ...
  IF r.status = 'RUNNING' THEN
    ...
    -- Auto-cashout is executed from database time, never from browser time.
    ...
    IF clock_timestamp() - r.started_at >= make_interval(secs => flight_seconds) THEN
      ...
      RETURN jsonb_build_object(
        ...
        'auto_cashouts',auto_result->'settled'
      );
    END IF;
