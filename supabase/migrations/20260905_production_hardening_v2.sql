...
-- ---------------------------------------------------------------------------
-- 1. Align bet states with the server engine.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 2. Remove legacy RPCs that trust client identity/multiplier.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 3. Secure place_bet: identity comes only from auth.uid().
--    The client supplies round + wager intent; the server validates everything.
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  IF p_amount < 0.50 OR p_amount > 5000.00 THEN
    RAISE EXCEPTION 'INVALID_AMOUNT';
  END IF;

  IF p_panel_id NOT IN (1,2) THEN
    RAISE EXCEPTION 'INVALID_PANEL';
  END IF;

  IF p_auto_cashout IS NOT NULL AND (p_auto_cashout < 1.01 OR p_auto_cashout > 1000) THEN
  ...
  -- A player can never create or mutate a round through this RPC.
  SELECT * INTO v_round
    FROM public.game_rounds
   WHERE id::text = trim(p_round_id)
   FOR UPDATE;
  ...
  -- One active bet per user/panel/round prevents accidental duplicate wagers.
  IF EXISTS (
    ...
    RAISE EXCEPTION 'ACTIVE_BET_EXISTS';
  END IF;
  ...
  INSERT INTO public.bets (
    id, round_id, user_id, amount,
    auto_cashout, auto_cashout_multiplier,
    status, panel_id, is_bot, created_at, updated_at
  ) VALUES (
    v_bet_id, v_round.id, v_user_id, p_amount,
    p_auto_cashout, p_auto_cashout,
  ...
  INSERT INTO public.transactions (
    ...
    v_tx_id, v_user_id, 'bet_placed', p_amount, 'USD',
    v_before, v_after,
  ...
  IF p_idempotency_key IS NOT NULL AND length(trim(p_idempotency_key)) > 0 THEN
    INSERT INTO public.idempotency_keys (
      user_id, idempotency_key, request_type, response_payload
    ) VALUES (
      v_user_id, trim(p_idempotency_key), 'place_bet', v_response
    )
...
-- ---------------------------------------------------------------------------
-- 4. Secure cashout: no multiplier and no user_id from the browser.
--    The server derives the current multiplier from the authoritative round.
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  IF NOT FOUND OR v_round.status <> 'RUNNING' OR v_round.started_at IS NULL THEN
    RAISE EXCEPTION 'ROUND_ENDED';
  END IF;

  -- Authoritative multiplier: elapsed server time, never a browser value.
  v_multiplier := GREATEST(
    1.00,
    floor((extract(epoch FROM (clock_timestamp() - v_round.started_at)) * 0.25 + 1.0) * 100) / 100
  );
  ...
  INSERT INTO public.transactions (
    ...
    v_tx_id, v_user_id, 'bet_cashed_out', v_payout, 'USD',
    v_before, v_after,
    'CASHOUT-' || v_round.round_number || '-' || v_bet.id,
    'completed', now(), now()
  );
...
-- ---------------------------------------------------------------------------
-- 5. Least-privilege RPC execution.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 6. Prevent direct player writes to financial/game tables.
--    Admin/server roles should use dedicated SECURITY DEFINER RPCs.
-- ---------------------------------------------------------------------------
...
-- Existing SELECT access is intentionally left untouched here; RLS policies
-- must be hardened separately after reviewing the complete live policy set.

-- ---------------------------------------------------------------------------
-- 7. Helpful uniqueness for active bet race protection.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 8. Wallet integrity: retain existing data and enforce non-negative balances.
-- ---------------------------------------------------------------------------
ALTER TABLE public.wallets
  DROP CONSTRAINT IF EXISTS wallets_available_balance_check;
ALTER TABLE public.wallets
  ADD CONSTRAINT wallets_available_balance_check
  CHECK (available_balance >= 0);

ALTER TABLE public.wallets
  DROP CONSTRAINT IF EXISTS wallets_locked_balance_check;
ALTER TABLE public.wallets
  ADD CONSTRAINT wallets_locked_balance_check
  CHECK (locked_balance >= 0);

-- NOTE: The server round engine, safe public round view, KYC storage policies,
-- support RLS, and admin-only RPCs are intentionally separate from this step.
-- Do not claim GO-LIVE until those components and the client game loop are
-- updated and validated against the live Supabase project.
